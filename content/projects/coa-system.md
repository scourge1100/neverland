...
CoA(Certificate of Analysis)는 반도체 패키징 공정에서 사용되는 자재(소모품·부품)의 품질 데이터다. 원래는 자재 공급업체가 이 데이터를 보내오면 품질팀이 취합해서 고객사인 S전자에 제출하는 방식으로 운영됐다.

문제는 이 과정에서 데이터가 사람 손을 한 번 거친다는 점이었다. S전자 입장에서는 품질팀이 취합 과정에서 데이터를 임의로 조작할 가능성을 배제할 수 없었고, 그래서 품질팀의 개입 없이 공급업체 데이터가 그대로 전달되는 구조를 요구했다. 처음 요구사항은 공급업체가 외부에서 사내 웹 시스템에 직접 접속해 데이터를 입력하는 방식이었지만, 내부망에 외부 접근 경로를 여는 것은 보안상 리스크가 너무 컸다. 그래서 공급업체별로 SFTP 계정을 발급하고, 스케줄러가 주기적으로 업로드 디렉토리를 확인해 엑셀 파일이 있으면 파싱해서 DB에 적재하는 구조를 대안으로 제시했다.

## 해결하려던 문제

- 엑셀 기반 수작업 프로세스로 인한 데이터 정확성·추적성 저하
- 품질팀이 데이터 취합 과정에 개입하면서 생기는 데이터 조작·왜곡 가능성 — S전자가 가장 우려한 지점
...
데이터 흐름은 업로드부터 검증, 승인, MES 반영까지 단계별로 분기한다. 검증 단계에서는 값이 정해진 임계치를 벗어나거나, 비어 있으면 안 되는 필수 항목이 공란인 경우를 감지해 실패로 처리하며, 모든 단계에서 메일 알림이 발생한다.
...
공급업체와 품질팀, 두 축의 흐름이 각각 진행되다가 주요 이벤트마다 메일 알림으로 합류하는 구조다. 승인된 데이터는 MES에 반영되는 것과 별개로, S전자와 협의된 날짜에 맞춰 취합돼 이메일로 발송된다 — 이 마지막 발송 단계에는 품질팀이 개입해 값을 고칠 여지가 없고, 정해진 스케줄러가 기계적으로 취합·발송한다.
...
flowchart TD
    ...
    subgraph 정기발송["정기 발송(스케줄러)"]
        C1[승인 데이터 취합] --> C2[JavaMail 발송]
        C2 --> C3[S전자]
    end
    B4 --> C1
    A1 -.notify.-> N[메일 알림]
...
sequenceDiagram
    ...
    Note over A: 협의된 발송일(스케줄러)
    A->>A: 승인 데이터 취합
    A->>SE: JavaMail로 취합 데이터 발송
```
...
- S전자와 협의된 발송일에 맞춰 승인 데이터를 자동 취합해 JavaMail로 발송하는 스케줄러 개발 — 품질팀 개입 없이 기계적으로 처리되도록 구현
...
기존의 반복적인 수작업 취합·검증 과정을 SFTP 업로드 기반 단일 프로세스로 완전히 대체했다. 초반 파일 포맷 오류를 해결하며 시스템을 안정화한 이후로는 큰 장애 없이 운영되고 있으며, 승인 절차와 오류 리포트가 자동화되면서 품질팀의 업무 부담이 줄고 보고 자료의 신뢰도와 전달 속도도 함께 개선됐다. 무엇보다 S전자가 가장 우려했던 지점 — 품질팀이 데이터를 취합하는 과정에서 값을 조작할 수 있다는 가능성 — 을 구조적으로 없앴다. 승인된 데이터가 사람 손을 거치지 않고 스케줄러에 의해 그대로 취합·발송되기 때문에, 내부 보안 정책을 지키면서도 데이터의 신뢰성을 증명할 수 있는 체계를 구현해 내부 보안 감사에서도 긍정적인 평가를 받았다.
